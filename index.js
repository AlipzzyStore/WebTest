...
async function startClient() {
    ...
    // cooldownStore simpan waktu terakhir eksekusi tiap endpoint
const cooldownStore = {
  delay: 0,
  ios: 0
};

// minimal 5 menit cooldown
const COOLDOWN_TIME_MS = 5 * 60 * 1000;

// middleware untuk ngecek cooldown global
function checkCooldown(type) {
  return (req, res, next) => {
    const now = Date.now();
    const lastUsed = cooldownStore[type];

    if (now - lastUsed < COOLDOWN_TIME_MS) {
      const sisa = Math.ceil((COOLDOWN_TIME_MS - (now - lastUsed)) / 1000);
      return res.status(429).json({
        status: false,
        message: `Cooldown aktif. Coba lagi dalam ${sisa} detik.`
      });
    }

    cooldownStore[type] = now; // update terakhir akses
    next();
  };
}

    app.get('/api/bug/forceall', async checkCooldown('forceall'), (req, res) => {
   ...
   app.get('/api/bug/ios', async checkCooldown('ios'), (req, res) => {
        const { target } = req.query;
        if (!target) return res.status(400).json({ status: false, message: 'parameter target diperlukan' });
    ...
    app.get('/api/bug/delay', async checkCooldown('delay'), (req, res) => {
        const { target } = req.query;
        if (!target) return res.status(400).json({ status: false, message: 'parameter target diperlukan' });
